...
class DQN:
    def __init__(self, params):
        ...
        if self.params['ckpt_file'] is not None:
            self.global_step = tf.Variable(int(self.params['ckpt_file'].split('_')[-1]), name='global_step',
                                           trainable=False)
        else:
        ...
        if self.params['ckpt_file'] is not None:
            ...
            self.saver.restore(self.sess, self.params['ckpt_file'])
                    #state   action  terminal  next_batch(next state?)    reward
